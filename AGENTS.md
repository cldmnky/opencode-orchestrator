...
- The repository is pinned to `@opencode/plugin` `2.0.14` and `@opencode/sdk` `2.0.14` for integration tests. Production code uses `Plugin.define` and the Promise plugin contract from the installed packages; do not mix package generations without updating the contract suite.
...
- Verify first: `bun run typecheck && bun test && bun run build && bun run scripts/package-smoke.ts`. `build` emits `dist/index.js`, `dist/tui.js`, `dist/commands.js`, `dist/installer.js`, `dist/cli/index.js`, and supported-entrypoint declarations; `dist/` is gitignored but included in `npm pack`.
...
- The plugin defaults to `strict_agents: true`; config-backed agents are materialized after external plugins during startup, so the complete dev template or installer is required for strict validation.
- `opencode api` requests default to the service location rather than the client `cwd`; scope location-aware requests explicitly with the OpenAPI deep-object parameter, for example `opencode api get '/api/plugin?location[directory]=/path/to/repo'`. The legacy `?directory=` parameter is ignored on 2.0.14.
