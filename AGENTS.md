...
- The repository is pinned to `@opencode/plugin` `0.0.0-beta-19507` and `@opencode/sdk` `0.0.0-beta-19507` for integration tests. Production code uses `Plugin.define` and the Promise plugin contract from the installed packages; do not mix package generations without updating the contract suite.
