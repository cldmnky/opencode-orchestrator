...
- The repository is pinned to `@opencode/plugin` `2.0.12` and `@opencode/sdk` `2.0.12` for integration tests. Production code uses `Plugin.define` and the Promise plugin contract from the installed packages; do not mix package generations without updating the contract suite.
...
- `opencode api` requests default to the service location rather than the client `cwd`; scope location-aware requests explicitly with the OpenAPI deep-object parameter, for example `opencode api get '/api/plugin?location[directory]=/path/to/repo'`. The legacy `?directory=` parameter is ignored on 2.0.12.
