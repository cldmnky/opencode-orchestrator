...
- **A fail-closed coherence signal for slices.** The additive D4 v2 signal names the explicit coherence question before work is split: a coupled outcome stays one `cohesive-slice`, an independent slice is only a `parallel-candidate` after the parent verifies exact disjoint write scopes from repository facts, and overlapping or unknown coupling is `serialized`. An unknown coherence answer falls back to collect-facts instead of guessing. It is advisory prompt-and-classifier guidance only — no host tool, command, config key, or gate is added — and it leaves D4 v1 and the D2 handoff contract unchanged.
...
- The additive D4 v2 coherence signal is prompt-and-classifier guidance only: no host tool, command, config key, or gate is added, D4 v1 and the D2 handoff contract are unchanged, and its runtime effect stays unproven — live-host telemetry is still `not-collected` and is never estimated.
