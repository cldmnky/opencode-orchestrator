...
- **Truthful capability boundaries.** Prompts distinguish guidance, recorded claims, observed facts, and enforced gates. `max_parallel` is enforced for configured-role dispatches per root session in this process, but is not a scheduler or cross-process limit; publication revision and remote-state gates are enforced.
...
- **A durable lead board behind goals and plan runs.** `/goal` and `/run-plan` continuations advance a persisted task DAG — dependency-ordered reservations, conservative receipt recovery across restarts, and lead-only completion after plugin-observed shell receipts match the required checks and an approved exact-revision review. Scope packets are advisory, not isolation, and the board is durable but not transactional or exactly-once.
- **A read-only Orchestrator sessions sidebar and progress detail.** When the host exposes session tabs, the TUI sidebar lists the sessions running your configured orchestrator with each session’s live status (`busy` / `running` / `idle`, or `unknown` when no state is known), cost, and bounded goal/board/review progress. The **View orchestrator progress** palette action opens bounded task, budget, worktree, publication, and gate details. The CLI plugin reads this through a typed server RPC and never reads server storage directly; missing or incomplete state stays visibly unknown.
...
3. **Bounded review mode** — `review.mode: "bounded"` (default `"prompt"`). Every publication step requires an exact-revision approved V2 review receipt with plugin-observed reviewer-child provenance; `review_get` / `review_start` are lead-only and `review_submit` is reviewer-only. These tools are registered only in `bounded` mode, so with the default `"prompt"` no review record can exist and publication **fails closed** until `review.mode: "bounded"` is enabled. Legacy V1 records remain readable as `legacy-unproven` status but never authorize publication.
