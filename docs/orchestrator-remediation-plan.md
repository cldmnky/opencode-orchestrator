...
- [x] Phase 6 runtime parallel dispatch admission (with documented 2.0.11 completion limitation).
...
measured the pinned 2.0.11 shell hook, added bounded plugin-observed
...
host-contract findings and any item that remains advisory because 2.0.11
...
The repository is pinned to `@opencode/plugin` and `@opencode/sdk` `2.0.11`. Any change that depends on host behavior must be proven against that pinned version before production code relies on it.
...
treated as proven unless 2.0.11 delivers the corresponding after event.
