...
**Status:** Design artifact + **callable advisory classifier (issue #10 implementation)** · **Artifact:** D4 · **Date:** 2026-08-30
...
**Runtime implementation:** `src/core/d4.ts` (`classifyTaskComplexity`) exposed as the orchestrator-only tool `orchestrator_task_complexity_classify` (`src/opencode-v2/orchestration/tools.ts`); tests in `test/unit/d4.test.ts` and `test/unit/orchestration-tools.test.ts`.
**Companion artifacts:**

- [`d4-gate-table.json`](./d4-gate-table.json) — the decision table (machine-readable, mirrored by the runtime classifier)
- [`d4-task-corpus.json`](./d4-task-corpus.json) — synthetic, non-measured task corpus (frozen)
...
The repository already nudges users toward the answer informally: `README.md:30` says to skip the orchestrator for single-file trivial edits, and `src/core/policy.ts` (`orchestrationRules`, lines 91-115) describes disjoint write scopes, serialization of overlapping implementation, and review-before-completion as **prompt guidance only**. The gate formalizes that advisory logic into a decision table so it can be evaluated on a corpus before any runtime code is considered.

**This is a recommendation layer, not an enforcement layer (and it stays that way in the runtime implementation).** Issue #10 implemented the table as a callable classifier (`classifyTaskComplexity` in `src/core/d4.ts`, exposed as `orchestrator_task_complexity_classify`), but it still does **not** block or force delegation, does not change how a task is executed, and cannot override an explicit user decision. Every result carries `advisory: true`, and `runtimeEnforced: false` is preserved in the JSON table.

The classifier input is the **eight structured dimension facts** (each optional/nullable); it does **not** infer facts from prose. Fact assessment therefore remains the caller's/evaluator's responsibility: whoever calls the tool must first collect and assert the eight facts. This is exactly the boundary the frozen evaluation protocol encodes (feed the gate its features and record the recommendation before reading the reference label).
...
2. **`high-risk` → `orchestrate-with-review`** — any high-risk signal wins over every other label. Review is already the default obligation for orchestrated runs (`require_review=true`, `src/core/config.ts:52`; `src/core/policy.ts:110`).
...
**The labels are hypotheses, not measurements.** No case has been executed; there is no latency, token, or quality data attached to any case in this phase. The corpus is **frozen and unmeasured** — `docs/phase-1/d4-task-corpus.json` and `docs/phase-1/d4-evaluation-template.json` are not part of the issue #10 write scope and were not changed. Runtime conformance was checked read-only against the implemented classifier: 11 of 12 cases classify to their `referenceRecommendation`; `d4-case-006` is the **documented intentional mismatch** — its `shared_mutable_state: true` feature drives the classifier's precedence rule 2 (`orchestrate-serialized`) against the case's `orchestrate-candidate` adjudication hypothesis, and that tension is exactly what the evaluation protocol is meant to measure.
...
2. For each case, assess the request into the eight dimension facts, feed **only those structured features** to the gate, and record the recommendation before ever reading the reference label. (The implemented classifier `orchestrator_task_complexity_classify` accepts structured features only — it does not read raw request text — so the evaluator remains responsible for the fact assessment that step 2 describes.)
...
- The gate is implemented as a **callable classifier** (`classifyTaskComplexity`, tool `orchestrator_task_complexity_classify`) but remains advisory — `runtimeEnforced: false` in the JSON table; every result is `advisory: true`; no part of the runtime delegates, blocks, or enforces.
...
**This phase has collected no results and no telemetry.** The implemented classifier computes deterministic recommendations from structured facts (advisory only), but nothing here is a measurement: the corpus is synthetic and unexecuted, the evaluation template's result fields are null or `"not-collected"` by design and remain frozen, and no latency/token/quality number is claimed. Nothing in `docs/phase-1/` claims measured behavior, runtime enforcement, or validated accuracy for the gate (corpus conformance checks in `test/unit/d4.test.ts` verify the classifier honors the frozen table and its documented case-006 mismatch — they are not an evaluation run).
...
| Prompt-only policy | `src/core/policy.ts:91-115` | Disjoint write scopes, serialization of overlapping implementation, review before completion — guidance only, which is why the gate is also advisory |
...
| Review default | `src/core/config.ts:52`, `src/core/policy.ts:110` | `require_review=true` — review obligation for orchestrated runs |
| Runtime classifier | `src/core/d4.ts` | `classifyTaskComplexity` — strict input schema over the eight facts, precedence order, `advisory: true` |
| Validation tool | `src/opencode-v2/orchestration/tools.ts` | `orchestrator_task_complexity_classify` registration, orchestrator-only runtime check |
| Conformance tests | `test/unit/d4.test.ts` | Corpus conformance (case006 intentional mismatch); `test/unit/orchestration-tools.test.ts` covers the tool |
...
- No runtime enforcement, hard blocking, or automatic delegation — the classifier is advisory (`runtimeEnforced: false`, `advisory: true`).
- No measured corpus results, no telemetry, no accuracy claims; the corpus and evaluation template remain frozen.
...
- No inference of facts from raw request prose: the tool accepts the eight structured facts only, and fact assessment stays with the caller/evaluator.