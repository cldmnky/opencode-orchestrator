...
export const MANAGED_WORKTREE_GUIDANCE = [
  ...
  "The tools are orchestrator_worktree_list, orchestrator_worktree_create, orchestrator_worktree_status, orchestrator_worktree_sync, orchestrator_worktree_enter, orchestrator_worktree_push, and orchestrator_worktree_cleanup.",
  "A missing tracked worktree is the normal initialization state before creation; it is not unavailable managed-worktree support.",
  "Never use a bare git worktree list as an integration probe: it lists Git checkouts, not the plugin tool family or current-session managed records.",
  "When the worktree tools are exposed and no current-session record exists, use orchestrator_worktree_list or orchestrator_worktree_status, then create the managed worktree instead of reporting support unavailable.",
...
export const WORKTREE_LIFECYCLE_GUIDANCE = [
  ...
  "A missing record is a create-next condition, not a blocker; do not delegate until creation produces a ready tracked worktree and enter returns entered:true.",
...
export const STRUCTURED_HANDOFF_GUIDANCE = [
  ...
  "Handoff validation applies only after a worker returns a handoff; it is never a pre-implementation or managed-worktree-creation service.",
