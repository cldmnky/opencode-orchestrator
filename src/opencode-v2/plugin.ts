              ...
              ...(options.worktree.enabled
                ? [
                    "Use orchestrator_worktree_list, orchestrator_worktree_create, orchestrator_worktree_status, orchestrator_worktree_sync, orchestrator_worktree_enter, orchestrator_worktree_push, and orchestrator_worktree_cleanup only for the current session's managed worktree; delegated children get no atomic isolation.",
                     "Worktree lifecycle is enabled and orchestrator-owned: implementation delegation MUST be preceded by orchestrator_worktree_create -> orchestrator_worktree_enter -> delegate to the implementer. orchestrator_worktree_enter moves only the current session into its tracked worktree (session ID and history preserved); children delegated afterward inherit or start from that context. Only the orchestrator creates, enters, pushes, and cleans up managed worktrees. A pending or failed worktree_enter result is not a successful receipt: wait for the V2 safe boundary and retry until entered:true before delegating. When the worktree tools, a whitelisted worktree.root, allow_mutations, a ready tracked worktree, or a successful worktree_enter are unavailable, stop and ask the user instead of delegating implementation from the main checkout.",
                  ]
                : []),
