 ...
 * behavior change. Every probe runs in an embedded 2.0.14 host against a
 ...
 *   - Worktree operations are project-scoped in 2.0.14. The harness activates
...
describe("phase B native worktree contract (pinned 2.0.14)", () => {
  ...
  test("discovers externally created worktrees and drops vanished directories without an orphan state", async () => {
    ...
    try {
      await withHost(fixture, async ({ probe, domain, at }) => {
        ...
        // explicit refresh; list reads the saved inventory in 2.0.14.
