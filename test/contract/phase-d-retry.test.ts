 ...
 * (pinned `@opencode/plugin`/`@opencode/sdk` `2.0.14`).
 *
...
describe("phase D retry-hook contract (pinned 2.0.14)", () => {
  ...
  test("bounds attempts at the built-in maximum even when the hook keeps retrying", async () => {
    await withProbeHost(async ({ host, probe, directory }) => {
      ...
      // OpenCode 2.0.14 permits ten physical retries (attempts 2..11).
